...
import (
	...
	"github.com/PeerFintech/gohfc/pkg/parseBlock"
...
func (c *FabricClient) CreateUpdateChannel(identity Identity, path, channelId, orderer string) error {
...
func (c *FabricClient) JoinChannel(identity Identity, channelId string, peerName, orderer string) (*PeerResponse, error) {
	ord, ok := c.Orderers[orderer]
	if !ok {
...
func (c *FabricClient) InstallChainCode(identity Identity, req *InstallRequest, peerName string) (*PeerResponse, error) {
...
func (c *FabricClient) InstantiateChainCode(identity Identity, req *ChainCode, peers []string, orderer, operation, policy string,
	collectionsConfig []CollectionConfig) (*orderer.BroadcastResponse, error) {
	ord, ok := c.Orderers[orderer]
	if !ok {
		return nil, ErrInvalidOrdererName
	}
	...
	prop, err := createInstantiateProposal(identity, req, operation, policy, collConfigBytes, c.Crypto)
	if err != nil {
		return nil, err
	}
...
func (c *FabricClient) ListenForFullBlock(ctx context.Context, identity Identity, startNum int64, eventPeer, channelId string, response chan<- parseBlock.Block) error {
...
func (c *FabricClient) ListenForFilteredBlock(ctx context.Context, identity Identity, startNum int64, eventPeer, channelId string, response chan<- EventBlockResponse) error {
	ep, ok := c.EventPeers[eventPeer]
	if !ok {
		return ErrPeerNameNotFound
	}
...
func newFabricClientFromConfig(config ClientConfig) (*FabricClient, error) {
	...
	var err error
	if config.CryptoConfig.Family != "ecdsa" && config.CryptoConfig.Family != "gm" {
	...
	for name, p := range config.Peers {
		newPeer, err := NewPeerFromConfig(context.Background(), config.ChannelConfig, p, crypto)
		if err != nil {
			return nil, err
		}
		newPeer.Name = name
	...
	for name, p := range config.EventPeers {
		newEventPeer, err := NewPeerFromConfig(context.Background(), config.ChannelConfig, p, crypto)
		if err != nil {
			return nil, err
		}
	...
	for name, o := range config.Orderers {
		newOrderer, err := NewOrdererFromConfig(context.Background(), config.ChannelConfig, o)
		if err != nil {
			return nil, err
		}
